...
# SPECIFICATION: Build a Single Layer Perceptron and a Multi-Layer Perceptron Classifiers, then
# find the combination of two hyperparameters (learning rate and shuffle) that leads  the best 
# prediction performance for each classifier.
        ...
        for a in range(2): #iterates over the algorithms
            ...
            if a == 0:
                if accuracy > highest_perceptron_accuracy:
                    ...
                    print("Highest Perceptron accuracy so far: %s, Parameters: learning rate=%s, shuffle=%s" % (highest_perceptron_accuracy, w, b))
            else:
                if accuracy > highest_mlp_accuracy:
                    highest_mlp_accuracy = accuracy
                    print("Highest MLP accuracy so far: %s, Parameters: learning rate=%s, shuffle=%s" % (highest_mlp_accuracy, w, b))
